...
const reportDiscordToolDescription = "Deliver one complete standalone user-facing response. Set message to the natural response you want to deliver; it may be ordinary conversation, a greeting, an answer, a question, an action, current progress, status, a finding, or a conclusion. Do not force conversational content into status language. Describe work directly without internal execution details, waiting, or a promised later update. Omit skipReport or set it to false to record message. Set skipReport=true only when there is no meaningful user-facing response at all; a normal greeting or conversational response is meaningful and must not be skipped. If the call is rejected, use the tool-result feedback and retry with corrected arguments that preserve the intended content and tone while removing disallowed internal details."
...
- useful ongoing progress is valid user-facing content and must be phrased as the current action;
- the message does not describe internal execution, waiting, or a promised later update;
	...
	return agentcli.Tool{
		Definition: agentcli.ToolDefinition{
			...
			InputSchema: agentcli.ObjectSchema(struct {
				...
				Message:    agentcli.StringParameter("Complete standalone user-facing response; ordinary conversation, greetings, answers, questions, progress, and results are all valid; do not describe internal execution, waiting, or future updates; when skipReport is true, briefly state why no user-facing response is necessary (the message will not be recorded)").Required().MinLength(1).MaxLength(maximumDiscordMessageRunes),
