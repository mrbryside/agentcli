...
const subagentOutcomeRepairReminder = `This child turn attempted to finish without a successful report_subagent_outcome result. Do not repeat the delegated work and do not repeat any domain action or tool call. Review the existing messages and tool results, then call report_subagent_outcome exactly once. Use completed only when every required action is resolved; omit next_step and error. Use incomplete when required work or information remains; include one concrete next_step and omit error. Use failed only when a terminal error prevents continuation; include the actual error, omit next_step, and never invent recovery work. This is a bounded repair loop; call the outcome tool again on the next repair if this attempt does not produce a successful result.`
