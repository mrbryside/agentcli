	...
	tests := []struct {
		...
		},
		{
			...
			contains: []string{
				...
				"required subagent results are ready",
				"executed=false",
				"Treat that call as handled",
		...
		},
		{
			...
			contains: []string{
				"Application description.",
				"every tool in the same batch succeed",
				"current turn ends",
			},
			excludes: []string{"complete final response"},
		},
		{
			...
			contains: []string{
				"complete final response",
				"final-response action executes successfully",
				"current turn ends",
				"required subagent results are pending",
			},
