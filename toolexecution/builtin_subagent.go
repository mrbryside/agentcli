...
type SubagentController interface {
	...
	Send(context.Context, string, string, string) (storage.Subagent, error)
...
func (bridge *SubagentToolBridge) Tools() []Tool {
	return []Tool{
		...
		bridge.tool(SendSubagentMessageToolName, "Send a focused follow-up to an identified existing child. Resolve the user's friendly display name to its id through active_subagents. If the child is busy the message is queued; otherwise a new asynchronous child turn starts. Do not poll after sending it: finish the parent turn or do other useful work, and use the next callback as the result.", `{"type":"object","properties":{"subagent_id":{"type":"string"},"message":{"type":"string","description":"Focused missing detail, clarification, recovery request, or next task"}},"required":["subagent_id","message"],"additionalProperties":false}`, bridge.send),
...
func (bridge *SubagentToolBridge) start(ctx context.Context, arguments json.RawMessage) (json.RawMessage, error) {
	...
	return json.Marshal(struct {
		...
		Asynchronous bool                   `json:"asynchronous"`
		NextAction   string                 `json:"next_action"`
	}{record.ID, record.DisplayName, record.SessionID, record.CurrentTurnID, record.Status, result.Action, result.Action == SubagentStartReused, true, "The child turn is running asynchronously. Address it by display_name when speaking to the user, use its id for tools, and do not poll. Continue useful independent work or finish the response; completion is delivered separately."})
}
...
func (bridge *SubagentToolBridge) status(ctx context.Context, arguments json.RawMessage) (json.RawMessage, error) {
	...
	for _, record := range records {
		...
		return json.Marshal(SubagentStatusResult{
			...
			ResultReady:     record.Status == storage.SubagentStatusIdle && record.LastTurnID != "" && record.LastTurnError == "",
		})
	}
...
func (bridge *SubagentToolBridge) send(ctx context.Context, arguments json.RawMessage) (json.RawMessage, error) {
	...
	record, err := controller.Send(ctx, invocation.SessionID, input.ID, input.Message)
	if err != nil {
		return nil, err
	}
