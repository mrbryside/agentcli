...
func (bridge *TaskToolBridge) description() string {
	...
	description.WriteString("Run one focused task with a configured agent. For a new task, provide agent, description, and prompt. To continue any existing task later, provide its exact task_id and a prompt; agent and description are unnecessary. The presence of task_id always selects resume mode, and the runtime ignores agent or description if they are accidentally included. Tasks with completed, incomplete, or error results remain available for optional later continuation. An unknown, closed, or running task_id returns error_code task_not_found, task_closed, or task_running and never creates a replacement task. After any resume error, keep the same task_id when correcting the call; never remove task_id merely to make the call succeed. Foreground is the default: its final output is returned by this call. Use background only when later delivery is appropriate. Put independent tasks in the same assistant tool-call message so they can run in parallel. Do not call task again merely to check progress or make tool calls solely to delay a response.")
