...
func (bridge *SubagentToolBridge) Tools() []Tool {
	return []Tool{
		...
		bridge.tool(SendSubagentMessageToolName, "Send one focused follow-up to an existing child selected by ID. Use it for new user information, a clarification, a missing detail requested by an incomplete callback, or a distinct next task. A successful result means accepted or queued, not completed. The runtime accepts at most one message from the same parent turn to the same child; exact retries are deduplicated and changed retries return already_sent. After dispatch, do not call status/list/close to wait. End the parent turn or continue independent work; use the later callback as the outcome.", `{"type":"object","properties":{"subagent_id":{"type":"string","minLength":1,"description":"Stable ID of the existing child, resolved from its display_name in active_subagents."},"message":{"type":"string","minLength":1,"description":"One focused follow-up containing the new information or next task. Do not send a waiting/status request."}},"required":["subagent_id","message"],"additionalProperties":false}`, bridge.send),
