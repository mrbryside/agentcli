...
func (bridge *TaskToolBridge) Tools() []Tool {
	return []Tool{{
		Definition: agentruntime.ToolDefinition{
			...
			InputSchema: mustRawToolSchema(`{"type":"object","properties":{"agent":{"type":"string","minLength":1,"description":"Exact configured agent name for a new task."},"description":{"type":"string","minLength":1,"description":"Short label describing a new task."},"prompt":{"type":"string","minLength":1,"description":"Complete instructions and context for this task turn."},"task_id":{"type":"string","minLength":1,"description":"Existing task ID to resume in the same main-agent session."},"background":{"type":"boolean","description":"Run without waiting for the final result. Omit or use false to return the result in this turn."}},"required":["prompt"],"additionalProperties":false}`),
		},
...
func (bridge *TaskToolBridge) description() string {
	...
	description.WriteString("Run one focused task with a configured agent. For a new task, provide agent, description, and prompt. To continue the same task later, provide task_id and prompt; omit agent and description. Foreground is the default: its final output is returned by this call. Use background only when later delivery is appropriate. Put independent tasks in the same tool batch so they can run in parallel. Do not use task for polling or simulated waiting.")
