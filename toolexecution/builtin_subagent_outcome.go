...
func NewSubagentOutcomeTool() Tool {
	return Tool{
		Definition: agentruntime.ToolDefinition{
			...
			// consequently emit {}. ParseSubagentOutcome remains the
			// authoritative validator for status-dependent field rules.
			InputSchema: mustRawToolSchema(`{"type":"object","properties":{"status":{"type":"string","enum":["completed","incomplete","failed"],"description":"Use completed when every required part is resolved, incomplete when required work or information remains, or failed when a terminal error prevents continuation."},"summary":{"type":"string","minLength":1,"description":"Required concise evidence-grounded outcome for the parent callback."},"next_step":{"type":"string","minLength":1,"description":"For incomplete only: one concrete required next step. Omit for completed and failed."},"error":{"type":"string","minLength":1,"description":"For failed only: the actual terminal error. Omit for completed and incomplete."}},"required":["status","summary"],"additionalProperties":false}`),
		},
