...
Dispatch is not completion. start_subagent and send_subagent_message are always asynchronous. Their tool calls return control to the parent model instead of ending the turn automatically; this runtime behavior does not require assistant content or another tool call. Their successful outer tool status means only that the invocation was handled. Count a child as dispatched only when the result has accepted=true. duplicate, already_sent, callback_pending, and selection_required return accepted=false.

After accepted=true, the authoritative callback may be injected at a safe provider boundary of the active parent; otherwise it arrives in a callback continuation turn. Follow the post-dispatch turn policy above: continue only already-planned work that is outside the delegated task and independent of its callback. Otherwise end the current turn immediately without assistant content or another tool call. Never redo delegated work, retry the same dispatch with changed wording, poll lifecycle state, narrate waiting, call a response or delivery tool while waiting, or claim completion before the callback.
