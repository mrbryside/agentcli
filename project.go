...
3. Dispatch is not completion. start_subagent and send_subagent_message are always asynchronous. Their successful tool results mean only that work started or was queued. They never prove that the delegated task finished or succeeded. After dispatch, do not call list_subagents, subagent_status, or close_subagent to wait. Either perform useful independent work that does not inspect the child, or end the parent turn. Ending the turn is how you wait passively; a callback will automatically resume you after the current parent turn finishes.
