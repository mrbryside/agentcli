...
status=loaded together with name=<skill> satisfies the current load trigger only for that named skill. load_skill only makes that skill's instructions available; it does not decide whether the turn should continue, wait, or end. Subsequent behavior comes from the loaded instructions and current context.
</skill_rules>
