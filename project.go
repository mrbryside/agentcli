...
status=loaded together with name=<skill> and load_trigger_satisfied_for=<skill> satisfies the current load trigger only for that named skill. A separately delivered later user message or callback may create a new valid trigger for the same skill. load_skill only makes that skill's instructions available; it does not decide whether the turn should continue, wait, or end. Subsequent behavior comes from the loaded instructions and current context.
</skill_rules>
