...
3. Dispatch is not completion. start_subagent and send_subagent_message are always asynchronous. Their successful dispatch results mean only that work started or was queued, never that the delegated task finished. The runtime stores the result and ends the parent turn automatically; a callback later starts a separate parent turn. Do not add a speculative answer or question beside a dispatch tool call, and do not call list_subagents, subagent_status, or close_subagent to wait. A start_subagent result with selection_required is the exception: no dispatch occurred, so ask the user which display_name to continue with.
