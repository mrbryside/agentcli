...
`)
	...
	for _, expected := range []string{"only agent allowed", "Children never receive subagent-management tools", "<subagent_orchestration_rules>", "Dispatch is not completion", "start_subagent and send_subagent_message always continue", "exactly one start_subagent call per provider round", "Never batch multiple start_subagent calls", "requested definition", "accepted=true", "required finalizer", "wait for authoritative callbacks", "already-planned independent work", "neither duplicates delegated work nor depends on callback results", "callback_action=wait_existing", "callback_action=none", "Never redo a delegated task", "Never poll", "Callbacks are authoritative", "new_instance=true", "incomplete children remain open", "automatically closes completed and failed children", "one-shot system reminder", "close_subagent is destructive", "latest human user message explicitly", "Never choose it autonomously", "user_instruction"} {
