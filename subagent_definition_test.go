...
`)
	...
	for _, expected := range []string{
		...
		"ends the current turn automatically after the successful tool batch",
		...
		"Never mix values",
		"any false call that returns a pending callback ends the entire successful tool batch",
		"## Dispatch result handling",
		"Dispatch is not completion",
		"start_subagent and send_subagent_message are always asynchronous",
		"start_subagent ends a successful pending-callback tool batch",
		"send_subagent_message returns control to the parent model",
		...
		"independent of every pending callback",
		"make no more tool calls or assistant content",
		"Do not call a response or delivery tool",
