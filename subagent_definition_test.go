...
`)
	...
	for _, expected := range []string{"only agent allowed", "Children never receive subagent-management tools", "<subagent_orchestration_rules>", "Dispatch is not completion", "force_close_subagent always continue", "exactly one start_subagent call per provider round", "Never batch multiple start_subagent calls", "requested definition", "accepted=true", "required finalizer", "wait for authoritative callbacks", "already-planned independent work", "neither duplicates delegated work nor depends on callback results", "callback_action=wait_existing", "callback_action=none", "Never redo a delegated task", "Never poll", "Callbacks are authoritative", "new_instance=true", "Never close a running or incomplete child", "close_subagent always continues with a normal provider round", "reuse the same child", "callback must be consumed", "force_close_subagent is destructive", "latest user message explicitly", "Never choose it autonomously"} {
