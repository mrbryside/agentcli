...
`)
	...
	for _, expected := range []string{"only agent allowed", "Children never receive subagent-management tools", "<subagent_orchestration_rules>", "Dispatch is not completion", "finish_turn, which defaults to true", "continue decomposing", "final dispatch", "selection_required always continues", "callback_pending is also a successful controlled result", "Never retry that child", "operations already planned for other children", "Never poll", "Callbacks are authoritative", "new_instance=true", "Never close a running or incomplete child", "always continues the current turn", "reuse the same child", "callback must be consumed", "force_close_subagent is destructive", "latest user message explicitly", "Never choose it autonomously"} {
