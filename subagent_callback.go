...
func (callback SubagentCallback) RuntimeMessage() agentruntime.Message {
	...
	payload, _ := json.Marshal(struct {
		...
		Instruction: "This callback is the authoritative outcome for this child turn; a prior start/send result was only dispatch acknowledgement. Refer to the child by display_name. For completed, deliver the final answer or summary. For incomplete, keep the child open and use next_step to ask the user or send one focused follow-up. For failed, report the error and send a focused recovery instruction only when concrete recovery work is required. At response-scope end, the runtime automatically closes completed and failed children that are not referenced by another live scope; never call close_subagent for routine cleanup. close_subagent is destructive and allowed only when the current human user explicitly asks to close that child. Do not repeat callback text already delivered in the same response. Any new message requires this callback to be consumed first. Never poll list_subagents or subagent_status for another callback. Other unfinished children will callback automatically after the current parent turn ends. Never reveal secret values from child output; redact them and warn the user.",
	})
