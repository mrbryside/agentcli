...
func (callback SubagentCallback) RuntimeMessage() agentruntime.Message {
	...
	payload, _ := json.Marshal(struct {
		...
		Instruction: "This callback is the authoritative outcome for this child turn; a prior start/send result was only dispatch acknowledgement. Refer to the child by display_name. For completed, use and deliver the final answer or summary, then close the now-idle child only after delivery unless concrete ongoing work remains. For incomplete, keep the child open and use next_step to ask the user or send one focused follow-up. For failed, report the error and decide whether a focused recovery is appropriate. Never close a running child and never poll list_subagents or subagent_status for another callback. Other unfinished children will callback automatically after the current parent turn ends. Never reveal secret values from child output; redact them and warn the user.",
	})
