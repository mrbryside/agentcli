...
func (callback SubagentCallback) RuntimeMessage() agentruntime.Message {
	...
	payload, _ := json.Marshal(struct {
		...
		Instruction: "This callback is the authoritative outcome for this child turn; a prior start/send result was only dispatch acknowledgement. Refer to the child by display_name. For completed, deliver the final answer or summary, then either reuse the child for a distinct next task or close it when no concrete work remains. For incomplete, keep the child open and use next_step to ask the user or send one focused follow-up. For failed, report the error, then either send one focused recovery instruction or close the child when no recovery is needed. Any new message or close requires this callback to be consumed first. Never close a running or incomplete child. Never poll list_subagents or subagent_status for another callback. Other unfinished children will callback automatically after the current parent turn ends. Never reveal secret values from child output; redact them and warn the user.",
	})
