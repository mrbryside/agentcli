...
func (callback SubagentCallback) RuntimeMessage(progress ...toolexecution.ResponseScopeCallbackProgress) agentruntime.Message {
	...
	payload, _ := json.Marshal(struct {
		...
		Instruction:      "This is an authoritative subagent result. Read callback_progress before deciding what to do. pending_callbacks identifies delegated work whose result is still outstanding; never duplicate, replace, retry, or poll it. received_callbacks identifies results already delivered in this response scope; process each exactly once. If results remain outstanding, continue only specific already-planned work that is independent of every outstanding result and will not duplicate or invalidate delegated work. If no such work remains, stop this turn without assistant content or another tool call; never call a tool to simulate waiting. The runtime resumes the parent automatically when another result is ready. When no results remain outstanding, combine all delivered outcomes before finishing. For completed, use final_answer or summary. For incomplete, use next_step for one focused follow-up or user question. For failed, report the error and recover only when concrete work remains.",
	})
