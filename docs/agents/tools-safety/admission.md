...
main agent. Permission and confirmation requests remain owned by the subagent
executor, but both lifecycles are emitted as main agent-addressed events while the
requests remain in their durable stores for recovery. The main agent UI resolves
them with `ResolveSubagentPermission` or `ResolveSubagentConfirmation`; a
completion result is deliberately not used because the subagent cannot complete
...
and subagent permissions and confirmations share one visible oldest request, and
