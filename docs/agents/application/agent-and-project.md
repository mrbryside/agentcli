...
`curl -fsSL https://raw.githubusercontent.com/mrbryside/agentcli/main/install.sh | sh -s -- [directory]` runs the optional project bootstrapper. It prompts through the terminal for a Go module path (or accepts `--module`) and creates a minimal terminal application plus `.agentcli` configuration, an example skill, and a researcher subagent. The generated application intentionally has no executable tools; applications must register those in Go code.
