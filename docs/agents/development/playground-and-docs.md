...
compaction, provider, model-limit, reasoning, and generic `extra_body`
settings. Model limits remain commented so the configured provider `/models`
endpoint or `models.dev` resolves them first; users enable the manual values
only for an unknown custom model. The opening banner formats whichever context
limit was resolved. Those tools and their tests belong in `playground/terminal`,
not in the reusable `agentcli` package.
