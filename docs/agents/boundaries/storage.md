...
`storage.Message` is the canonical provider-neutral transcript value. It stores identity, session, turn, type, text, generic tool calls, and generic tool results. User/system/runtime-event content must be nonblank; tool calls/results have distinct validation rules. Never persist OpenAI SDK message types.
