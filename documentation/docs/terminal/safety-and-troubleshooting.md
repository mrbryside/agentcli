...
## Permission or confirmation appears while viewing a task session

The request retains the task session, turn, and tool-call correlation. You may
answer it in the selected task-session view or return to the main-agent view
and use its ID:
...
The Terminal queues main-agent and task-session permissions and confirmations together and
...
and task-agent output are intentionally isolated.
